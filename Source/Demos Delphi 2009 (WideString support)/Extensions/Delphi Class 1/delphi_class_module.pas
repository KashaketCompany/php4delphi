...
var
  AnsiScriptText : AnsiString;
begin
  inherited;
  if FScriptText <> ''  then
   begin
     AnsiScriptText := AnsiString(FScriptText);
     zend_eval_string(PAnsiChar(AnsiScriptText), nil, 'delphi', ts_resource(0));
   end;
end;
...
begin
  if FScriptText <> Value then
   begin
     ...
     Move(Value[1], FScriptText[1], L * SizeOf(Char));
   end;
end;
